...
function Vehicules() {
  ...
  const filteredVehicules = vehicules.filter(v => v.nom.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className="flex-1 relative overflow-auto z-10 bg-gray-100 min-h-screen py-8">
      ...
      <main className="container max-w-7xl mx-auto px-4">
            ...
            <table className="w-full text-left text-gray-700 border-separate border-spacing-y-2">
              ...
              <tbody>
                {filteredVehicules.map((vehicule, index) => (
                  <motion.tr key={vehicule.id} initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: index * 0.05 }} className="bg-white rounded-lg shadow-sm hover:shadow-lg transition-all duration-200">
                    <td className="px-6 py-4">
                      {vehicule?.images?.[0]?.path ? (
                        <img src={vehicule.images[0].path} alt="car" className="w-10 h-10 rounded-full object-cover border"/>
                      ) : (
                        <img src="/brabus.jpg" alt="Profil" className="w-10 h-10 rounded-full object-cover border"/>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-block px-3 py-1 rounded-full text-sm font-semibold mb-4 ${statusConfig[vehicule.status]?.class}`}>
                    ...
                    <td className="px-6 py-4 flex gap-2">
                      ...
                      <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className="text-red-500 hover:text-red-700" onClick={() => handleDelete(vehicule.id)}><BiTrash size={20} /></motion.button>
                    </td>
                  </motion.tr>
                ))}
              </tbody>
        ...
        <Pagination currentPage={pagination.currentPage} lastPage={pagination.lastPage} onPageChange={page => getVehicules(page)} />
      </main>

      {openModal && (
        <VehiculeForm vehicule={selectedVehicule} onClose={handleCloseModal} isEdit={!!selectedVehicule} />
      )}
    </div>
  );
}
